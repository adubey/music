...
/**
  * Represent a probability distribution over chords
  *
  * V1
  */
class ChordTable(val table : Map[Chord, PriorityQueue[ChordProb]]) {
  ...
  var nextSample : Option[Chord] = Some(Chord("C E G"))

  def sample : Option[Chord] = {
    val currentSample = nextSample
    val p = r.nextDouble
    for (sample <- currentSample) {
      nextSample = get(sample, p)
    }
